...
for bin in cat cp date du grep gzip head ls mkdir mv rm sed sort tee touch tar; do
  [ "$(which "$bin")" ] || missing="$missing $bin"
done; [ "$missing" ] && {
...
DATA=$TMP/gt5.html
BDIR=~/.local/share/gt5x
DDIR=~/.local/share/gt5x/diffs
CUT_AT="0.1"
MAX_DEPTH=5
SAVE_DU=1
ML=10000
...
gt5_help() {
  ...
  echo "   --benchmark     save this run as a benchmark diff, which will be preserved"
  ...
  echo "   --num-diffs i   specify the i diff files to keep [$NUM_DIFFS]"
  echo "   --save-as file  DEPRECATED, use 'du', see manpage"
  echo "   --save-state    overwrite a previous '--discard'"
  echo "   --show-tree i   display the directory/file structure as a tree upto i levels, 0 is all levels (requires 'tree')" [$TREE_LEVEL]
...
while [ "$1" ]; do
  ...
  case "$1" in
    ...
    --num-diffs)
      if [[ "$2" =~ ^[0-9]+$ ]]; then
        NUM_DIFFS=$2
        shift  # Shift past the number just processed
      else
        echo "bad num-diffs-arg \"$2\", valid is non-negative integer"
        exit 1
      fi
      ;;
    ...
    --show-tree)
      ...
      if [[ "$2" =~ ^[0-9]+$ ]]; then
        ...
        echo "Optional depth level for --show-tree should be a non-negative integer."
        exit 1
      fi
      ;;
...
sanitizedPath=$(echo "$PWD" | sed 's/[\/]/_/g') 
if [ "$BENCHMARK" = true ]; then
  GENERIC_STORE="$DDIR/${sanitizedPath}-BENCHMARK-$timestamp.tar.gz"
else
  GENERIC_STORE="$DDIR/${sanitizedPath}-$timestamp.tar.gz"
fi
[ -d "$DDIR" ] || mkdir -p "$DDIR" || exit
...
# Insert tree data into HTML if --show-tree option was used
if [ "$SHOW_TREE" = true ]; then
  if ! which tree > /dev/null; then
    echo "The 'tree' command is required for this option but is not installed."
    echo "Please install 'tree' using your distribution's package manager."
    exit 1
  fi

  if [ "$TREE_LEVEL" -eq 0 ]; then
    TREE_OUTPUT=$(tree -C "$CD")
  else
    TREE_OUTPUT=$(tree -C -L "$TREE_LEVEL" "$CD")
    echo "$TREE_OUTPUT"
  fi

  echo "<h2>Directory Structure</h2>" >> "$DATA"
  echo "<pre>$TREE_OUTPUT</pre>" >> "$DATA"
fi

echo "<div style='text-align:center; margin-top:50px;'>gt5x</div>" >> "$DATA"
...
[ "$SAVE_DU" ] && {
  ...
  tar -czf "${GENERIC_STORE}.$$.tmp" -C "$TMP" \
  	--transform="s/^this/$(basename "${GENERIC_STORE}" .tar.gz)/" this & 
  ...
  # Cleanup old diffs, keeping only the specified number, excluding benchmarks
  START_LINE=$((NUM_DIFFS + 1))
  ls -t "${DDIR}/"*-*.tar.gz | grep -v "BENCHMARK" | tail -n +"$START_LINE" | xargs -r rm --
}
...
[ "$SAVE" ] && {
  ...
  cp -l $DATA $BDIR/.gt5.html 2> /dev/null || cp $DATA $BDIR/.gt5.html || exit
  $BROWSER $DATA
