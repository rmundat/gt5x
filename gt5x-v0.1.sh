...
DDIR=~/.local/share/gt5x/gt5-diffs
CUT_AT="0.1"
MAX_DEPTH=5
SAVE_DU=1
ML=10000
...
gt5_help() {
  ...
  echo "   --diff-bench f  diff run against bechmark, if no file is specified use last" 
...
[ "$F1" ] && unset SAVE_DU || {
  ...
  if [ "$DIFFBENCH" = true ]; then
    if [ -z "$DF" ]; then
      DF=$(find "$DDIR" -type f -name '*BENCHMARK*.tar.gz' -print0 | xargs -0 /bin/ls -t | head -1)
    fi
  else
    unset DF
    DF=$(/bin/ls -t $F1-*.tar.gz 2>/dev/null | head -1)
  fi
...
[ "$SAVE_DU" ] && {
  ...
  find "${DDIR}" -maxdepth 1 -type f -name '*-*.tar.gz' ! -name '*BENCHMARK*' -print0 \
   | xargs -0 -r /bin/ls -t | tail -n +"$START_LINE" | xargs -r rm --
